setGeneric("munge", signature = "object", 
    function(object, ...) standardGeneric("munge"))
setMethod("munge", "NanoStringExperiment", 
function(data, mapping = update(design(data), exprs ~ 
    .), extradata = NULL, elt = "exprs", ...) {
    ...
    if (hasGeneMatrix || hasSignatureMatrix) {
        df <- data.frame(SampleName = sampleNames(data), sData(data), check.names = FALSE, 
    ...
    if ((!hasAggregates && !hasAssayDataElts) && (hasFeatureVars || useSignatures) && hasSampleVars) 
    ...
    if (hasAssayDataElts) {
        ...
        if (useSignatures) {
            df <- summary(data, MARGIN = 1L, log2scale = hasLog2Summaries, elt = elt, signatureScores = TRUE)
            df <- df[, intersect(vars, colnames(df)), drop = FALSE]
            df <- copyRowNames(df, "SignatureName")
        }
        else {
            MARGIN <- 1L + hasSampleVars
            df <- summary(data, MARGIN = MARGIN, log2scale = hasLog2Summaries, elt = elt)
            df <- df[, intersect(vars, colnames(df)), drop = FALSE]
            if (MARGIN == 1L) {
    ...
    if (useSignatures && is.null(df)) {
        df <- data.frame(SignatureName = names(signatures(data)), stringsAsFactors = FALSE)
    }
    if (hasSampleVars) {
